...
{		
	if (flag == true || node.file_loc == 0)
	{
		...
		cout << node.key_value << "  " << node.left_child << " " << node.right_child << " " << node.height << " " << node.parent << " " << node.file_loc << endl;
		if (node.file_loc != C_NULL)
		{
			file.seekg(node.file_loc * C_NODE_SIZE);
 ...
 * It takes a root from the insert, the file, the temporary nodes
