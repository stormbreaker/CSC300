...
{		
	...
	cout << "adjusted " << left.key_value << "height " << left.height << endl;	
	return flag;		
}
void bal(avl_tree_node &root, fstream &file, avl_tree_node &left, avl_tree_node &right, avl_tree_node &node)
{
	int rootheight = 0;
	int otherheight = 0;
	cout << endl << root.key_value << endl;
	cout << endl << left.key_value << endl;
	cout << endl << right.key_value << endl;
	if (root.left_child == C_NULL)
	{
		left.height = C_NULL;
	}
